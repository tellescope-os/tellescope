...
const SubmitButton = ({ formik, onClick, submitText="Submit", style={ marginTop: 5, width: '100%' } }: SubmitButton_T) => (
  <Button color="primary" variant="contained" type="submit" onClick={SUPPORTS_FORMS ? undefined : onClick}
      disabled={formik.isSubmitting || !formik.isValid || !formik.dirty} style={style}
    >
    <Typography component="span">{submitText}</Typography>
    {formik.isSubmitting && <CircularProgress size={11} style={{ marginLeft: 5, marginBottom: 1 }}/>}
  </Button>
)
    ...
    <Formik initialValues={initialValues} validationSchema={Yup.object(validationSchema)} onSubmit={async (v, o) => {
      ...
      {formik => (
        <Flex column flex={1}>
          ...
          <Flex column>
            <Flex>
              <SubmitButton formik={formik} onClick={formik.handleSubmit} {...options}/>
            </Flex>
            <Flex>
